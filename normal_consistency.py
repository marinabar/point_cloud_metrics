...
gen_files = [f for f in os.listdir(deepcad_gen_path) if f.endswith('.stl')]
...
for f in gt_files:
    ...
    gen_mesh = transform_real_mesh(gen_mesh)
    ...
    auc_gen, median_cos_sim_gen, per_inval = compute_normals_metrics(gen_mesh, gt_mesh)

    results.append({
        'filename': f,
        'auc_generated': auc_gen,
        'median_cos_sim_generated': median_cos_sim_gen,
        'percentage_invalid_neighbors': per_inval
    })
    print(f"Processed: {f}")

mean_auc = np.mean([res['auc_generated'] for res in results])
mean_cos_sim = np.mean([res['median_cos_sim_generated'] for res in results])
mean_per_inval = np.mean([res['percentage_invalid_neighbors'] for res in results])

print(f"Mean AUC: {mean_auc:.4f}, Mean Median Cosine Similarity: {mean_cos_sim:.4f}, Mean Percentage Invalid Neighbors: {mean_per_inval:.2f}%")

results_df = pd.DataFrame(results)
results_df.to_csv('normal_metrics_results.csv', index=False)
print("Results saved to normal_metrics_results.csv")
